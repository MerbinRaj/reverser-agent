...
async def lifespan(app: FastAPI):
    ...
    load_dotenv()
    global reverser_chain
    try:
        llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", google_api_key=os.getenv("GEMINI_API_KEY"))
        prompt_template = PromptTemplate(
            input_variables=["text"],
            template="Reverse the following word and give only the final result: {text}"
        )
        reverser_chain = LLMChain(llm=llm, prompt=prompt_template)
        print("✅ Word reverser model loaded successfully!")
    except Exception as e:
        print(f"❌ Error initializing word reverser model: {e}")
    yield
...
async def chat(user_query: str = Form(""), image: UploadFile = File(None)):
    if image and image.filename:
        raise HTTPException(status_code=400, detail="Image processing is not supported. Please provide a word to reverse.")
    elif user_query:
        try:
            reversed_word = await reverser_chain.arun(text=user_query)
            return JSONResponse({"response": reversed_word})
        except Exception as e:
            return JSONResponse({"response": f"Error reversing word: {e}"})
    else:
